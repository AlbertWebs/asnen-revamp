...
@section('content')
    <div class="mb-4 flex items-center justify-between">
        @can('events.create')
            <a href="{{ route('admin.events.create') }}" class="rounded-md bg-forest-700 px-4 py-2 text-sm font-medium text-white hover:bg-forest-800">New</a>
        @endcan
    </div>
                ...
                @forelse ($events as $item)
                    <tr>
                        ...
                        <td class="px-4 py-3 text-right text-sm">
                            @can('events.update')
                                <a href="{{ route('admin.events.edit', $item) }}" class="text-forest-700 hover:underline">Edit</a>
                            @endcan
                        </td>
                    </tr>
                @empty
