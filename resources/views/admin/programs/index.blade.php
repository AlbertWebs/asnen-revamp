...
@section('content')
    <div class="mb-4 flex items-center justify-between">
        @can('programs.create')
            <a href="{{ route('admin.programs.create') }}" class="rounded-md bg-forest-700 px-4 py-2 text-sm font-medium text-white hover:bg-forest-800">New</a>
        @endcan
    </div>
                ...
                @forelse ($programs as $item)
                    <tr>
                        ...
                        <td class="px-4 py-3 text-right text-sm">
                            @can('programs.update')
                                <a href="{{ route('admin.programs.edit', $item) }}" class="text-forest-700 hover:underline">Edit</a>
                            @endcan
                        </td>
                    </tr>
                @empty
