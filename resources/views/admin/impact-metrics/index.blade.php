...
@section('content')
    <div class="mb-4 flex items-center justify-between">
        @can('impact_metrics.create')
            <a href="{{ route('admin.impact-metrics.create') }}" class="rounded-md bg-forest-700 px-4 py-2 text-sm font-medium text-white hover:bg-forest-800">New</a>
        @endcan
    </div>
        ...
        <table class="min-w-full divide-y divide-charcoal-200">
            ...
            <tbody class="divide-y divide-charcoal-100">
                @forelse ($metrics as $item)
                    <tr>
                        <td class="px-4 py-3 text-sm font-medium text-charcoal-900">{{ $item->question }}</td>
                        ...
                        <td class="px-4 py-3 text-right text-sm">
                            @can('impact_metrics.update')
                                <a href="{{ route('admin.impact-metrics.edit', $item) }}" class="text-forest-700 hover:underline">Edit</a>
                            @endcan
                        </td>
                    </tr>
                @empty
